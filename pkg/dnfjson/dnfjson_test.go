...
func TestDepsolver(t *testing.T) {
	...
	for tcName := range testCases {
		t.Run(tcName, func(t *testing.T) {
			...
			deps, _, sbomDoc, err := solver.Depsolve(pkgsets, tc.withSbom)
			if tc.err {
				assert.Error(err)
			...
			if tc.withSbom {
				...
				spdxDoc, err := sbomDoc.ToSpdxDocument()
				assert.NoError(err)
				assert.NotNil(spdxDoc)
				assert.Len(spdxDoc.Packages, len(deps))
			} else {
				assert.Nil(sbomDoc)
			}
		})
...
`
	...
	pkgSpec, repoCfg, _, err := solver.Depsolve(nil, false)
	assert.NoError(t, err)
