...
func ec2ImageConfig() *distro.ImageConfig {
	...
	// systemd-firstboot in el7 does not suppoer --keymap option
	...
	ec2UserSudoers, err := fsnode.NewFile("/etc/sudoers.d/ec2-user", nil, nil, nil, []byte("ec2-user\tALL=(ALL)\tNOPASSWD: ALL\n"))
	if err != nil {
		panic(err)
	}
