...
func Depsolve(
	...
	// the GPG keys from the blueprint customizations.
		...
		for txIdx, pkgSet := range pkgSetChain {
			...
			for _, pkgName := range pkgSet.Include {
				...
				pkg := rpmmd.Package{
					...
					// NOTE: we can't use version higher than 4, because some pipeline generators are searching
					// for DNF4 packages by version.
